...
class SafariRide {
  constructor(data) {
    ...
    this.people = data.people || data.passengers || 0;
  ...
  static async create(rideData) {
    try {
      ...
      const normalized = {
        ...
        parkName: rideData.parkName || '',
        people: rideData.people || rideData.passengers || 0,
      ...
      const rideRef = db.collection('safariRides').doc(newId);
  ...
  const savedDoc = await db.collection('safariRides').doc(rideRef.id).get();
  ...
  static async getAll() {
    try {
      const ridesSnapshot = await db.collection('safariRides')
  ...
  static async getById(rideId) {
    try {
      ...
      let rideDoc = await db.collection('safariRides').doc(rideId).get();

      // If not found, try to find a document whose readableId or bookingId matches the provided id
      if (!rideDoc.exists) {
        const q1 = await db.collection('safariRides').where('readableId', '==', rideId).limit(1).get();
        if (!q1.empty) rideDoc = q1.docs[0];
      }

      if (!rideDoc.exists) {
        const q2 = await db.collection('safariRides').where('bookingId', '==', rideId).limit(1).get();
  ...
  static async update(rideId, updateData) {
    try {
      ...
      let rideRef = db.collection('safariRides').doc(rideId);
